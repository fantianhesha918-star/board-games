...
- トランプ
  ...
  - 七並べ: 実装済み（CPU対戦 2〜4人 / 2人対面、パス回数 3/4/5、ジョーカー=ワイルドの あり/なし）。ジョーカーは好きな空きマスに置ける“ワイルド”（1ゲーム1回きり・戻せない）で、置いた位置の本物カードを持つ人は次の手番で必ずそれを出す（パス不可、実カードに置き換わる）。バーストで手札を場に開く、順位表示
