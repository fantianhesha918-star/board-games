...
- データ構造: `rooms/{4桁コード}` に `{game, status, host:{uid,name}, guest:{uid,name}|null, state, createdAt, updatedAt, hostSeen, guestSeen}`
- オンライン対応はオセロ・神経衰弱。**共通の約束**：`state.turn` は 0=ホスト / 1=ゲスト、`state.winner` は `"h"`/`"g"`/`"draw"`（未決着は null）。この約束に合わせて作れば `firestore.rules` は変更不要（othello だけ歴史的に turn:"b"/"w"・winner:"b"/"w" の特例）。`state` の中身はゲーム依存（othello=`{board,turn,last,pass,winner}`、memory=`{deck,taken,up,turn,scores,winner,seq}`）
- ルール変更時は Firebaseコンソール（Firestore Database → ルール）に `firestore.rules` を手貼りして「公開」（自動デプロイなし。firebase-tools CLI は未導入＝アカウント全体のアクセス権をPCに置くのを避けるため、2026-08-31 ユーザー判断）
...
- トランプ（オセロ以外の4種は「人間の人数 1〜4 ＋ CPUの人数」を設定し合計2〜4人。人間2人以上は交代プレイ、手札を隠すゲームは手番交代ごとに「次の人に渡す」画面をはさむ）
  - 神経衰弱: 実装済み（2〜4人・人間/CPU混在可、CPU記憶力3段階、12/20/30枚）。盤は全員に見えるので交代画面なし。CPUは各自が独立した記憶を持つ。**オンライン対戦（2人）対応**：「オンライン」モードで部屋作成/参加。盤は共有なのでやり取りするのはめくった位置・スコア・手番のみ。2枚めくった側の端末がペア判定して結果を送信
